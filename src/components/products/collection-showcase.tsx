            ...
            {filters.map((filter) => {
              ...
              return (
                <button
                  key={filter.value}
                  className={[
                    "rounded-full px-5 py-3 text-sm uppercase tracking-[0.18em] transition-all duration-300",
                    isActive
      ...
      {filteredProducts.length === 0 ? (
        ...
        >
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-stone-600 sm:text-base">
            Adjust the filter or search term to discover more handmade pieces
            from Art Gallery by Sneha.
          </p>
        </motion.div>
      ) : null}
    </div>
